...
/**
 * EMMA is being built one step at a time. The only functionality so far is
 * login verification: the run signs in to the Maryland procurement portal and
 * confirms the session, and this panel reports the outcome. Search + export
 * come next.
 */
  ...
  return (
    <div className="space-y-6">
      ...
      <Card
        ...
        description="This first step signs in to EMMA with the credentials in server/.env and confirms the session. Nothing is scraped yet — solicitation search and Excel export are added next."
      >
          ...
          {run.status === "failed" && (
            <div className="space-y-3 p-5">
              {run.errors.length > 0 ? (
                ...
                <ErrorBanner message="Login failed. Check the EMMA credentials in server/.env and try again." />
              )}
            </div>
          )}
        </section>
