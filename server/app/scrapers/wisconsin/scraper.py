...
class WisconsinScraper(BaseScraper):
    def __init__(self, run_id: str, keyword: str = "", agency: str = "", nigp_code: str = ""):
        ...
        # closing too soon, and rows kept despite an unreadable Due Date/Time.
    ...
    def scrape_all_pages(self) -> None:
        ...
        while True:
            ...
            for rec in self._scrape_page():
                ...
                # Keep only events still at least MIN_DAYS_UNTIL_CLOSE days from
                # their Due Date/Time; an unreadable due date is kept and tallied.
                days_left = days_until_close(rec.get("due_datetime"))
    ...
    def flush_partial(self) -> int:
        """Wisconsin's rows, named by the run's search criteria."""
        ...
        return partials.flush_records(
            ...
            sheet_name=f"Wisconsin_({search})",
        )
